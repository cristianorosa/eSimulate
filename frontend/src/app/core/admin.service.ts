...
export class AdminService {
  ...
  getAreasPaginated(page: number = 1, pageSize: number = 10): Observable<PaginatedResponse<Area>> {
    return this.http.get<PaginatedResponse<Area>>(`${this.apiUrl}/areas/paginated?page=${page}&page_size=${pageSize}`);
  }
  ...
  getExamsPaginated(page: number = 1, pageSize: number = 10, areaId?: number): Observable<PaginatedResponse<Exam>> {
  ...
  createQuestion(question: Question): Observable<Question> {
    return this.http.post<Question>(`${this.apiUrl}/questions/create`, question);
  }
  ...
  getOptions(questionId: number): Observable<Option[]> {
    return this.http.get<Option[]>(`${this.apiUrl}/options?question_id=${questionId}`);
  }
